...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	 ...
	 */
     ...
     if(found == 0){
        printf("Cannot play the next wave file. No available slot!");
     }
...
static void fillPlaybackBuffer(short *buff, int size)
{
    ...
    for(int i = 0; i < MAX_SOUND_BITES; i++){
        ...
        short* data = sound -> pData;
        int numSamples = sound -> numSamples;

        int samplesToMix = size;
        if(loc + samplesToMix > numSamples){
            samplesToMix = numSamples - loc;
        }

        for(short j = 0; j<samplesToMix;j++){
            int mixed = buff[j] + data[loc+j];
